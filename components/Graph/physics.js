...
export const configureSimulation = (fg, graphData) => {
  ...
  fg.d3Force('y', d3.forceY()
    .y(node => {
      ...
      if (node.type === 'recovered' && node.start_date) {
        // ✅ Posiciona recovered proporcionalmente ao ano, interpolado na linha principal
        const approxIndex = node.start_date / 50;
        return approxIndex * NODE_SPACING;
      }
      if (node.type === 'lost' && node.start_date) {
        return (node.start_date / 50) * NODE_SPACING;
      }
      return 300;
    })
    .strength(node => {
  ...
  fg.d3Force('x', d3.forceX()
    .x(node => {
      ...
      // ✅ Recovered ficam numa faixa lateral mais afastada e consistente
      const code = node.id.split('').reduce((acc, c) => acc + c.charCodeAt(0), 0);
      const side = code % 2 === 0 ? 1 : -1;
      const depth = node.traceDepth ?? 1;
      // Quanto mais fundo na cadeia, mais afastado lateralmente
      return side * (100 + depth * 18);
    })
    .strength(node => {
      if (node.type === 'root') return 1.0;
      if (node.type === 'pope') return 0.9;
  ...
  fg.d3Force('charge', d3.forceManyBody()
    .strength(node => {
      if (node.type === 'pope' || node.type === 'root') return -20;
      if (node.type === 'recovered') return -30; // ✅ Repele mais — não sobrepõe
      return -10;
    })
    .distanceMax(220)
  );
